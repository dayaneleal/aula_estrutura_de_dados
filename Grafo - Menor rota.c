...
void menu_mostrar(void) {
  system("cls");
  printf("Implementação do Algortimo de Dijsktra\n");
  printf("Opções:\n");
...
void grafo_criar(void) {
  ...
  //Se compilador falhou em alocar espaço na Memoria
  if (custos == NULL) {
    system("color fc");
    printf("** Erro: Memória Insuficiente **");
    exit(-1);
  ...
  do{
    ...
    if(origem) {
      ...
      do {
        printf("Custo (positivo) do vertice %d para o vértice %d: ", origem, destino);
        scanf("%d",&custo);
      } while (custo < 0);
